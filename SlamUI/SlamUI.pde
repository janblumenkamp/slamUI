...
void draw()
{
  ...
  if((map.getMapSizeXMM() != 0) && (map.getMapSizeXMM() != 0))
  {
    ...
    for (int y = mapscaled_sizeY - 1; y >= 0; y--)
    {
      for (int x = 0; x < mapscaled_sizeX; x++)
      {
        ...
        stroke(255 - map.map[x_scaled][y_scaled][0]);
        fill(255 - map.map[x_scaled][y_scaled][0]);
        point(x, y);
      }
    }
   ...
   /* line((int)(map.getRobPosXPx() / mapScaleFacX),
         ...
         (int)((map.getMapSizeYPx() - map.getRobPosYPx()) / mapScaleFacY) - 20 * cos((110 + ) * (PI / 180)));*/
